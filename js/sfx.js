...
(function () {
  ...
  function ensureCtx() {
    ...
    try {
      ...
      // A gentle limiter keeps the louder output from clipping/distorting
      // when several sounds overlap.
  ...
  function tone(opts) {
    ...
    const vol = opts.vol == null ? 0.42 : opts.vol;
  ...
  function melody(freqs, opts) {
    ...
    freqs.forEach((f, i) => tone({
      ...
      start: i * gap, dur, vol: opts.vol == null ? 0.42 : opts.vol,
    }));
  }
  ...
  function noise(opts) {
    ...
    const g = ctx.createGain();
    g.gain.setValueAtTime(opts.vol == null ? 0.45 : opts.vol, t0);
    g.gain.exponentialRampToValueAtTime(0.0001, t0 + dur);
  ...
  const SFX = {
    ...
    // --- Named effects -----------------------------------------------------
    click()    { tone({ freq: 660, type: 'square',   dur: 0.07, vol: 0.30 }); },
    flip()     { tone({ freq: 520, type: 'triangle', dur: 0.09, vol: 0.34, glide: 760 }); },
    pop()      { tone({ freq: 880, type: 'sine',     dur: 0.1,  vol: 0.48, glide: 1320 }); },
    move()     { tone({ freq: 300, type: 'sine',     dur: 0.06, vol: 0.22 }); },

    eat()      { tone({ freq: 520, type: 'square', dur: 0.08, vol: 0.38, glide: 880 }); },
    merge()    { melody([523, 784], { gap: 0.07, dur: 0.12, type: 'triangle', vol: 0.42 }); },

    whack()    { noise({ freq: 1200, dur: 0.1, vol: 0.5 }); tone({ freq: 240, type: 'square', dur: 0.08, vol: 0.3, glide: 120 }); },

    correct()  { melody([659, 988], { gap: 0.09, dur: 0.13, type: 'triangle', vol: 0.48 }); },
    match()    { melody([784, 1047, 1319], { gap: 0.08, dur: 0.12, type: 'triangle', vol: 0.42 }); },
    wrong()    { tone({ freq: 320, type: 'sawtooth', dur: 0.28, vol: 0.36, glide: 150 }); },

    start()    { melody([392, 523, 659], { gap: 0.08, dur: 0.12, type: 'triangle', vol: 0.42 }); },
    tick()     { tone({ freq: 700, type: 'square', dur: 0.05, vol: 0.28 }); },

    win()      { melody([523, 659, 784, 1047], { gap: 0.13, dur: 0.18, type: 'triangle', vol: 0.5 }); },
    gameover() { melody([523, 415, 311, 247], { gap: 0.16, dur: 0.22, type: 'sine', vol: 0.42 }); },
